    ...
    <div className="rounded-2xl border bg-white p-4 shadow-sm">
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed p-8 text-center transition ${
          isDragActive ? 'border-brand-500 bg-brand-50' : 'border-gray-300 hover:bg-gray-50'
        }`}
      >
        <input {...getInputProps()} />
        <Upload className="h-8 w-8 text-brand-600" />
        <p className="text-sm text-gray-600">Arrastra y suelta PDFs/MD/TXT aquí, o haz clic para seleccionar.</p>
        <p className="text-xs text-gray-400">Privado y local. Se indexan por fragmentos para búsqueda semántica.</p>
      </div>

      {files.length > 0 && (
        <div className="mt-4">
          <div className="mb-2 text-sm font-medium text-gray-700">Archivos seleccionados</div>
          <ul className="space-y-2">
            {files.map(f => (
              <li key={f.name} className="flex items-center justify-between rounded-lg border px-3 py-2">
                <div className="flex items-center gap-2">
                  <FileText className="h-4 w-4 text-gray-500" />
                  <span className="text-sm">{f.name}</span>
                </div>
                <button className="rounded-lg p-1 hover:bg-gray-100" onClick={() => remove(f.name)}>
